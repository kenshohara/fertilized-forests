...
namespace fertilized {
  ...
  class MultiClassHoughThresholdDecider
   ...
   public:
     ...
     *   use opt2 if the ratio of samples of most frequent class is < \ref hough_heuristic_ratio or
    ...
    void optimize_and_set_for_node(
      ...
      std::vector<uint> class_counts(n_classes);
      ...
      if (use_heuristic) {
        uint max_count = *std::max_element(std::begin(class_counts), std::end(class_counts));
        uint other_count = element_id_list.size() - max_count;
        if (!(static_cast<float>(other_count) /
...
#undef ADDITIONAL_SHARED_VARS
    ...
    bool operator==(const IDecider<input_dtype,
      ...
      const auto *rhs_c = dynamic_cast<MultiClassHoughThresholdDecider<input_dtype,
                                                                       feature_dtype,
      ...
      if (rhs_c == nullptr) {
        ...
        bool eq_n_classes = n_classes == rhs_c -> n_classes;
